...
class CircleLoss(torch.nn.Module):
  ...
  def forward(self, outputs, batch):
    ...
    hm_loss, cl_loss, off_loss, occ_loss = 0, 0, 0, 0
    for s in range(opt.num_stacks):
      ...
      occ_loss += self.crit_reg(output['occ'], batch['reg_mask'], batch['ind'], batch['occ']) / opt.num_stacks
      if opt.wh_weight > 0:
        if opt.dense_wh:
    ...
    loss = opt.hm_weight * hm_loss + opt.wh_weight * cl_loss + \
           opt.off_weight * off_loss +opt.occ_weight * occ_loss
...
class CircleTrainerWithOcc(BaseTrainer):
  def __init__(self, opt, model, optimizer=None):
    super(CircleTrainerWithOcc, self).__init__(opt, model, optimizer=optimizer)

  def _get_losses(self, opt):
    loss_states = ['loss', 'hm_loss', 'cl_loss', 'off_loss', 'occ_loss']
    loss = CircleLoss(opt)
